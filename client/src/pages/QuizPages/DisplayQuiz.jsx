...
const DisplayQuiz = () => {
  ...
  const answerCheck = async () => {
    ...
    try {
      ...
      newQuiz(allResult.updatedQuiz);
      if (!response.ok) {
        throw new Error("Failed to store results");
      }

      //storing results in state management in frontend
      newResults({
        quizResults: result,
        score: `${score}/${quiz_questions.length}`,
        correct: score,
  ...
  return (
    <>
      <div className="w-full min-h-screen bg-black text-green-400 font-mono flex flex-col items-center justify-center p-6 relative overflow-hidden z-999">
        ...
        >
          <h2 className="text-4xl font-bold text-green-300 border-b border-green-400 pb-3 uppercase tracking-wide">
            {quiz_details.topic} Quiz
          </h2>
          <p className="mt-3 text-lg text-green-300">
            Difficulty: {quiz_details.difficulty} | Type: {quiz_details.type} |
            Questions: {quiz_details.number_of_questions}
          </p>
        </motion.div>
        ...
        >
          <h3 className="text-xl font-semibold text-green-300 border-b border-green-400 pb-2">
          ...
          {currentQuestion.question_type === "Written" && (
            <textarea
              ...
              value={userAnswers[currentQuestion.question_number] || ""}
              className="w-full p-4 border border-green-500 rounded-xl bg-gray-800 text-green-200 outline-none focus:ring-2 focus:ring-green-400 shadow-inner resize-none"
            ...
            currentQuestion.options && (
              <div className="flex flex-col gap-3 mt-4">
                {currentQuestion.options.map((option, index) => (
                  <label
                    key={index}
                    className="flex items-center text-lg text-left cursor-pointer transform transition hover:scale-105 bg-gray-800 p-3 rounded-lg shadow-inner"
                  >
                    <input
                      onClick={() =>
                        handleAnswerSelect(
        ...
        <div className="flex gap-5">
          <motion.button
            ...
            className={`w-full max-w-4xl text-gray-900 font-bold p-4 rounded-xl mt-6 transition-all shadow-[0_0_20px_rgba(0,255,0,0.5)] relative z-10 ${
